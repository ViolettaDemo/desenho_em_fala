...
y = Math.floor(Math.random() * 600); document.getElementById("status").innerHTML = "Desenhando um retângulo."; drawRect = "set";
 ...
 function setup() { canvas = createCanvas(900, 600); } function draw() { if(drawCircle == "set") { radius = Math.floor(Math.random() * 100);
 círculo(x, y, raio);
 document.getElementById("status").innerHTML = "UCírculo desenhado. "; drawCircle = "";
